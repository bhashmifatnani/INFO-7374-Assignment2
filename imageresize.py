...
if __name__ == '__main__':
    ...
    for alg in algs:
        ...
        if recurrent:
            ...
            folders_src = [dir for dir in sorted(os.listdir(in_dir)) if os.path.isdir(os.path.join(in_dir, dir))]
            for j, f1 in enumerate(folders_src):
		if f1 == "images":
			print os.path.join(in_dir, f1)
			r = pool.apply_async(
				func=resize_img_folder,
				args=[os.path.join(in_dir, f1), os.path.join(current_out_dir, f1), alg])
			break
                folders = [dir for dir in sorted(os.listdir(os.path.join(in_dir,f1))) if os.path.isdir(os.path.join(in_dir,f1))]
		for i, folder in enumerate(folders):
		    if folder == "images":
                    	if i % every_nth is 0 or repeat is True:
				print os.path.join(in_dir, f1 + folder)
                        	r = pool.apply_async(
                            		func=resize_img_folder,
                            		args=[os.path.join(in_dir, f1 + "/" + folder), os.path.join(current_out_dir, f1 + "/" + folder), alg])

		    elif ".txt" in  folder:
			subprocess.call(["cp", os.path.join(in_dir, f1 + "/" + folder), os.path.join(current_out_dir, f1 + "/" + folder)])
        else:
